    ...
    return(
        <form className={styles.wrapper} onSubmit={handleSubmit}>
            ...
            <div className={styles.content}>
                ...
                <Input onChange={handleChange} value={formData.name} name="name" required style={{width: "100%"}} placeholder="Imie"/>
                <Input onChange={handleChange} value={formData.mail} name="mail" required style={{width: "100%"}}  placeholder='E-mail' type="email" />
                <div className={styles.checkBoxContainer} onClick={() => setFormData({...formData, isAdmin: !formData.isAdmin})}>Administrator? <FontAwesomeIcon className={styles.checkbox} icon={formData.isAdmin ? faCheckSquare : faSquare} /></div>
