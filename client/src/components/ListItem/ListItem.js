...
import { UpAndOpacity } from "../../animations/animations";
...
const ListItem = ({name, id, assignedFor, setIsModalOpen, tasks, status}) => {
    ...
    useEffect(() => {
        if(assignedFor === user._id.toString()){
            switch (status){
                case 'new':
                    setSecondButton(
                    <button className={styles.button3} onClick={() => {changeStatus('work', id); getContent('/tasks', currentContent, filters) }}>
                        Zacznij Prace <FontAwesomeIcon style={{marginLeft: '1rem'}} icon={faCheck}/>
                    </button>)
                    break;
                case 'work':
                    setSecondButton(
                    <button className={styles.button4} onClick={() => {changeStatus('end', id); getContent('/tasks', currentContent, filters) }}>
                        Zakończ Prace <FontAwesomeIcon style={{marginLeft: '1rem'}} icon={faCheck}/>
                    </button>)
                    break;
                case 'end':
                    setSecondButton(
    ...
    return(
        <motion.div variants={UpAndOpacity} initial="initial" animate="animate" className={styles.wrapper}>
            <div className={styles.title}>{name}</div>
            <div className={styles.buttons}>
                {tasks ? <>{secondButton}
                <Link to={`/tasks/${id}`}>
                    <button className={styles.button} onClick={() => {setIsModalOpen(true); }}>
                        Sprawdź <FontAwesomeIcon style={{marginLeft: '1rem'}} icon={faMagnifyingGlass}/>
                    </button>
                </Link></>
                :
                <Link to={`/users/${id}`}>
                    <button className={styles.button} onClick={() => {setIsModalOpen(true); }}>
                        Edytuj <FontAwesomeIcon style={{marginLeft: '1rem'}} icon={faUserPen}/>
                    </button>
                </Link>}
            </div>
        </motion.div>
